...
class Furniture(Interactable, ABC):
  ...
  def open(self):
    ...
    if self.closed:
      ...
      if not self.content:
        print("%s Malheureusement, vous ne voyez aucun objet de valeur ou utile." %desc)
      else:
...
class Inventory(Interactable):
  ...
  def examine(self):
    ...
    print("Vous ouvrez votre vieux sac de voyage. Vous y trouvez %s." %",".join([item.full_name for item in self.content]))
...
class GameManager():
  ...
  def print_full_desc(self, force=False):
    ...
    full_desc += ", ".join([fg(75) + "%s" %directions[card].name + reset for card, door in self.current_room.doors.items() if door])
    full_desc += "."
    if self.current_room.floor:
      full_desc += "\nSur le sol, vous voyez "
      full_desc += ', '.join([item.full_name for item in self.current_room.floor]) + ".\n"
    if self.current_room.content:
      full_desc += "\nLa piece contient également "
      full_desc += ', '.join([item.full_name for item in self.current_room.content if not item.hidden]) + "."
